...
$prompts_nonce = isset( $_POST['marketing_analytics_mcp_prompts_nonce'] )
	? sanitize_text_field( wp_unslash( $_POST['marketing_analytics_mcp_prompts_nonce'] ) )
	: '';

if ( $prompts_nonce && wp_verify_nonce( $prompts_nonce, 'marketing_analytics_mcp_prompts' ) ) {
		...
		switch ( $prompt_action ) {
			case 'create_prompt':
				...
				if ( '' === $prompt_name || '' === $prompt_label || '' === $prompt_description || '' === $prompt_instructions ) {
					echo '<div class="notice notice-error"><p>' . esc_html__( 'Please fill in all required fields.', 'marketing-analytics-chat' ) . '</p></div>';
					break;
				}

				if ( ! preg_match( '/^[a-z0-9-]+$/', $prompt_name ) ) {
					echo '<div class="notice notice-error"><p>' . esc_html__( 'Prompt name can only contain lowercase letters, numbers, and hyphens.', 'marketing-analytics-chat' ) . '</p></div>';
					break;
				}
				...
				if ( '' !== $arguments_raw ) {
					...
					if ( is_array( $decoded_arguments ) ) {
						...
						echo '<div class="notice notice-error"><p>' . esc_html__( 'Invalid JSON in Arguments field.', 'marketing-analytics-chat' ) . '</p></div>';
						break;
					}
				}
			...
			case 'delete_prompt':
				...
				if ( '' === $prompt_id ) {
					echo '<div class="notice notice-error"><p>' . esc_html__( 'Missing prompt ID.', 'marketing-analytics-chat' ) . '</p></div>';
					break;
				}
			...
			case 'import_preset':
				...
				if ( '' === $preset_key ) {
					echo '<div class="notice notice-error"><p>' . esc_html__( 'Missing preset key.', 'marketing-analytics-chat' ) . '</p></div>';
					break;
				}
							...
							<tr>
								...
								<td>
									...
									<form method="post" style="display: inline;">
										...
										<button type="submit" class="button button-link-delete" onclick="return confirm('<?php echo esc_js( __( 'Are you sure you want to delete this prompt?', 'marketing-analytics-chat' ) ); ?>');">
